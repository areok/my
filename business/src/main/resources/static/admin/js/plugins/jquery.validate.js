	...
	prototype: {
		...
		focusInvalid: function() {
			if ( this.settings.focusInvalid ) {
				try {
					...
					.trigger("focusin");
                    // debugger;
                    // $(this.findLastActive() || this.errorList.length && this.errorList[0].element || []).focus();
				} catch(e) {
		...
		elements: function() {
			...
			.filter(function() {
				...
				//类型是text的时候 name对应的所有的输入框都应该校验
//				if ( this.name in rulesCache  || !validator.objectLength($(this).rules()) ) {
				if ( (this.name in rulesCache && !$(this).attr('type')=="text") || !validator.objectLength($(this).rules()) ) {
					return false;
				}
