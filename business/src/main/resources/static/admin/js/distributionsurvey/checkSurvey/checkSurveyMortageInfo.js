  ...
  //地铁校验
    ...
    function railwayInfoChange(){
    	...
    	if(val == '1'){
    		...
    		$("input[name='guarantee.metro']").addClass('ignore').prop('readonly',true).next('.validate-tooltip').remove();
    		$("input[name='guarantee.station']").addClass('ignore').prop('readonly',true).next('.validate-tooltip').remove();
    	    $("input[name='guarantee.walk']").addClass('ignore').prop('readonly',true).next('.validate-tooltip').remove();
    	}
    }
    railwayInfoChange();
    ...
    $("#withOutRailWayInfo").click(function () {
        // $("input[name='guarantee.metro']").val('');
        // $("input[name='guarantee.station']").val('');
        // $("input[name='guarantee.walk']").val('');
        $("input[name='guarantee.metro']").val('');
        $("input[name='guarantee.station']").val('');
        $("input[name='guarantee.walk']").val('');
        document.getElementById("metro").readOnly=true;
    ...
    var saveMortinfoFunc = function(){
        ...
        $.ajax({
            url         : '/checkSurvey/saveCheckMortInfo',
            contentType : 'application/x-www-form-urlencoded',
            type        : 'post',
