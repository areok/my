$(function(){ //等界面加载完成后，执行下面内容
    (function (){ // 闭包，防止下面定义的变量泄露到整个页面
        var table = $('#checkSurveyTable');
        var cancelbuttons = table.find('button').filter(function(index){return $(this).html()=="业务取消"});
        var otherbuttons = table.find('button').filter(function(index){return $(this).html()!="业务取消"});
        ...
        cancelbuttons.on('click',function(){
            ...
            cms.messager.prompt('业务取消',function(e,remark){
                if(!remark){
                    cms.messager.alert('请填写取消业务原因!');
                    return false;
                }
                $.ajax({
